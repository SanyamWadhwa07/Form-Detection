...
def upload_file():
    ...
    if file:
        ...
        while os.path.exists(os.path.join(app.config['UPLOAD_FOLDER'], filename)):
            ...
            filename = f"formDetection{count}{file_extension}"
            count += 1
...
def download_file():
    ...
    if exercise_name == "bicepcurls" or exercise_name == "hammercurls":
    ...
    elif exercise_name == "benchtricepdips":
        exercises.bencHTricepDips(filename_input=filename)
        remove_file(filename=filename)
        result = convert_video_supported(exercises.output_file)
        response = send_file(result, as_attachment=True)
        performanceFile = exercises.performanceFile
        return redirect(url_for('detailedAnalysis'))
    elif exercise_name == "ropepushdowns" or exercise_name == "triceppushdown":
        exercises.tricepropepushdown(filename_input=filename)
        remove_file(filename=filename)
        result = convert_video_supported(exercises.output_file)
        response = send_file(result, as_attachment=True)
        performanceFile = exercises.performanceFile
        return redirect(url_for('detailedAnalysis'))
    elif exercise_name == "cablerows":
        exercises.cableRows(filename_input=filename)
        remove_file(filename=filename)
        result = convert_video_supported(exercises.output_file)
        response = send_file(result, as_attachment=True)
        performanceFile = exercises.performanceFile
    ...
    elif exercise_name == "tbarrows":
        exercises.tBarRows(filename_input=filename)
        remove_file(filename=filename)
        result = convert_video_supported(exercises.output_file)
        response = send_file(result, as_attachment=True)
        performanceFile = exercises.performanceFile
    ...
    elif exercise_name == "dumbbellrows" or exercise_name == "bentoverrows":
        exercises.dumbbellRows(filename_input=filename)
        remove_file(filename=filename)
        result = convert_video_supported(exercises.output_file)
        response = send_file(result, as_attachment=True)
        performanceFile = exercises.performanceFile
        ...
        exercises.deadlift(filename_input=filename)
        remove_file(filename=filename)
        result = convert_video_supported(exercises.output_file)
        response = send_file(result, as_attachment=True)
        performanceFile = exercises.performanceFile
